;; code from https://github.com/dfeltey/lwc2016.git
#lang racket

(provide raise-syntax-error*)

;; copied from unstable/error to avoid a dependency on the
;; unstable package
...
(define (raise-syntax-error* message0 stx sub-stx
  ...
  (let* ([source-stx (or stx sub-stx within-stx)]
         [who (or who
                  (let* ([maybe-id (if (stx-pair? stx)
                                       (stx-car stx)
                                       stx)])
                    (if (identifier? maybe-id)
                        (syntax-e maybe-id)
                        '?)))]
         [message
         ...
         [message
          (if (error-print-source-location)
              (string-append (source-location->prefix source-stx)
                             message)
              message)])
    (raise
...
(define (compose-error-message who message
                               ...
                               . field+detail-list)
  (define details
    (field+detail-list->table
     'compose-error-message field+detail-list null))
  ...
  (let* ([parts (apply append
                       (for/list ([detail (in-list details)])
                         (let* ([field+opts (car detail)]
                                [field (if (pair? field+opts)
                                           (car field+opts)
                                           field+opts)]
                                [options (if (pair? field+opts)
                                             (cdr field+opts)
                                             '())]
                                [value (cdr detail)])
         ...
         [parts (let loop ([continued continued-message])
                  (cond [(pair? continued)
                        ...
                        [(null? continued)
                         parts]))]
         [parts (list* message (if (null? continued-message)
                                   ""
                                   ";")
                       parts)]
         [parts (if who
